# -*- coding: utf-8 -*-
from DataImport.language_mapping import TRANSLIT_DICT
import sys
import re
import regex
...
def to_latin(string_to_transliterate, lang_code='ru'):
...
def to_cyrillic(string_to_transliterate, lang_code='ru'):
    ...
    # If we do support it, check if the implementation is not missing before proceeding.
        ...
        while index < length_of_string_to_transliterate:
            ...
            if (lang_code == 'sr' and (c == u'L' or c == u'l') and c_plus_1 == u'j') or \
                ...
                if (lang_code == 'bg' and (c == 'sh' or c == 'Sh' or c == 'SH') and string_to_transliterate[
                    index + 1] in u'Tt'):
                    index += 1
                    c += string_to_transliterate[index]
...
def is_cyrillic(s=''):
    if s == '':
        return False
    result = regex.search(r'\p{IsCyrillic}', s)
