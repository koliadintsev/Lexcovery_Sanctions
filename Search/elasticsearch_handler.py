...
SERVER_URL = "http://34.132.27.113:9200/" # Основа

# Password for the 'elastic' user generated by Elasticsearch
#ELASTIC_PASSWORD_DEBUG = "tjkwrXgtwvK8kmtxUqE2"
ELASTIC_PASSWORD_DEBUG = 'QB37tfNded112LGBoUsk'
...
def initialize_client():
    ...
    if DEBUG:
        client = Elasticsearch(
            SERVER_URL_DEBUG,
            ca_certs=HTTP_CA_FILENAME_DEBUG,
...
async def search_fuzzy_request(request, fuzziness="AUTO"):
    ...
    #s = Search().using(client).index("sanctions_usa").query(query)
    s.execute()
    for hit in s:
    ...
    for list_name in LIST_SANCTIONS:
        ...
        if list_name == "US":
            ...
            s = Search().using(client).index("sanctions_usa_cons").query(query)
        elif list_name == "UK":
            s = Search().using(client).index("sanctions_uk").query(query)
        elif list_name == "UK_Cons":
            s = Search().using(client).index("sanctions_uk_cons").query(query)
        elif list_name == "EU":
            ...
            s = Search().using(client).index("sanctions_au").query(query)
        elif list_name == "CA":
            s = Search().using(client).index("sanctions_ca").query(query)
        elif list_name == "CH":
            s = Search().using(client).index("sanctions_ch").query(query)
        s.execute()
        for hit in s:
            res = {"index": hit.meta.index, "score": hit.meta.score, "hit": hit}
            newlist.append(res)
    search_result = sorted(newlist, key=itemgetter('score'), reverse=True)
